...
| Project Importer / Agent Lab | Embeds the experimental `/agent-lab` workflow for importing real agent projects by local folder upload, ZIP upload, Git import, or mapped folder refresh; configuring providers; building/running containers (containerized, hybrid, or external-endpoint deployment modes); and launching authorised scans. Deploying an agent auto-produces a **working** scan target matching the agent's real HTTP contract — including `GET`/query and plain-text responses — on an auto-selected free host port with a run-mode-aware base URL, health-gated before registration. External/hybrid modes require an authorization acknowledgement. |

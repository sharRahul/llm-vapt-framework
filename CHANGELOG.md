...
- **Every assessment tool declares a contract.** A module used to be a `run(context, payloads) -> Finding` and nothing else: no argument schema, no availability check, no per-tool timeout or output cap, and no way to ask what it could assess before running it. `modules/contract.py` adds `ToolContract` — identifier, purpose, supported target types, argument schema, permissions, execution environment, timeout, output limit, result type, platform support, availability check. `ModuleRegistry.register` refuses a module without one or with a mismatched id, and `TestRunner` checks availability and target support before a module sends its first request. `core/process_boundary.py` is the single boundary for a tool declared as `subprocess`: argument arrays only, bounded by the contract's timeout, output truncated at its limit, and refused outright when the declared executable is absent.
- **A validated route from a proposed action to execution.** `core/tool_request.py` implements envelope → schema → target/scope → policy → adapter, with each rejection naming the stage that refused it. The default adapter executes nothing, which keeps the current behaviour exactly: a model still has no path to execution, and a model that can run commands remains a stated non-goal. The pipeline exists so the first feature that lets anything *propose* an action has one validated route rather than inventing its own — a model-originated request must carry a rationale, may not use subprocess or container execution, and may not carry `process.spawn`, `docker`, `filesystem.write`, or `network.egress`.
- **A findings burn-down with a real data source.** `GET /api/trends` aggregates open versus remediated findings per day across completed scans, and the dashboard chart that had been permanently empty now draws it. Only successful runs contribute — a cancelled or timed-out run stopped early, so its count is not a measurement — a quiet day carries the previous posture forward, and only `fixed` counts as remediated.
- **A console test suite.** Vitest and Testing Library cover the rules that decide what an operator sees: severity and status normalisation, risk scoring, the dashboard counters, terminal scan states, phase labels, triage justification, API error messages, and the empty states. `npm test` runs them and CI runs them on Python 3.12. Those rules moved out of `App.tsx` into `webui/console/src/lib/findings.ts` so they can be stated directly.
- **OpenAI and Anthropic provider presets.** Agent Lab offered Ollama, LM Studio, OpenRouter, and a custom OpenAI-compatible endpoint. OpenAI and Anthropic are now presets too; the Anthropic preset sets both the native and OpenAI-compatible variables, and the bundled `http_llm_agent` speaks Anthropic's Messages API (`x-api-key`, `anthropic-version`, content blocks) as well as the OpenAI-compatible shape. A template deploy takes a provider, base URL, model, and API key from the console, which it previously could not.
- **`analyst` is a real role.** It carried exactly the permissions of `viewer`. It now carries `start_configured_scan` — it can run a scan an admin configured, and still cannot change what a target is.
...
- **A template deploy always built VulnoraIQ's own image.** `docker build -f Dockerfile <context>` resolves `-f` against the server's working directory, not the build context, so every Agent Lab template built the repository root Dockerfile and failed on a Dockerfile that was never meant for it. A template's `dockerfile` now resolves against its context, as in Compose.
- **Removing a hosted agent left its scan target behind.** The target kept reporting `ready` for a container that no longer existed, and failed only once a scan was started. Removal now deletes the targets the deploy registered — Agent Lab already did this for its own deployments.
- **API failures were shown as the raw JSON envelope.** A Docker build failure reached the operator as a single line of `{"error": "...
..."}` with every newline escaped. The console reads the server's `error` field, so a failure is readable.
- **Editing a target could silently repoint it.** A target stored with `endpoint` and no `base_url` was shown with the *new-target* default `http://127.0.0.1:9090`, and saving would have written that. The editor resolves `endpoint` to `base_url` and inherits no endpoint-shaped scaffolding from a new target.
- Saving a new target no longer jumps the editor to an unrelated one, and a target created or deleted in the Targets view now reaches the header scan selector without a page reload.
- The scan-target selector is no longer hidden below 640 px while **Run Scan** stays enabled, so a scan cannot be started against a target the operator cannot see.
- The workspace no longer tells an operator with no scans to adjust a filter they never set.
- The finding pane no longer prints the same mitigation sentence twice, renders a "Compliance" heading over nothing, or lets a long assistant explanation push the evidence below the fold.
...
- **The release packager shipped git-ignored local files.** It walked the filesystem against a hand-maintained deny list, so a developer's `webui/console/node_modules` (99 MB) and `docs/owasp-pdfs/` (66 MB) were copied straight into the artifact — and any future ignored directory, a local `.env` included, would have leaked the same way. Selection is now intersected with `git ls-files`, which is the authoritative answer to what belongs in the project. A linux package went from 76 MB / 6691 files to 0.7 MB / 295 files, with tests asserting both that nothing untracked ships and that everything required still does.
- The container smoke test could not start its own container. It ran the image in production mode bound to `0.0.0.0` without proxy trust, which the production gate correctly refuses, and it printed no container logs — so the failure read as ten identical "connection refused" lines. It now configures the container the way a hardened deployment is configured, prints the container's logs when it fails to serve, and additionally asserts the fail-closed property the gate exists for: production mode must refuse to start with no admin token.
- GitHub Actions bumped off the deprecated Node 20 runtime (`checkout@v5`, `setup-python@v6`, `setup-node@v5`, `upload-artifact@v7`, `download-artifact@v7`), and CI installs Node 22.
